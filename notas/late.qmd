...
Con esta notación podemos escribir la variable de impacto de $i$ como $Y_i(Z,D)$.
...
para todo par de vectores de asignación $C$ y $C'$ **con el mismo número de tratados**.
...
Este supuesto indica que cualquier efecto de la asignación sobre los resultados debe ocurrir **a través** del cumplimiento, y no por otra vía:
...
Es decir, la exclusión permite escribir $Y$ en función solo de $D$.
...
Los factores $(1)$, $(0)$ y $(-1)$ son los tres valores que puede tomar $D_i(1)-D_i(0)$. Al condicionar en cada uno de ellos, ese segundo factor del producto se vuelve una constante y sale de la esperanza. Cada valor corresponde a un tipo de individuo: $(1)$ a los **cumplidores**, $(0)$ a quienes la asignación no mueve —**nunca cumplidores** y **siempre adoptadores**— y $(-1)$ a los **retadores**.

La intuición de este valor esperado es obtener el valor esperado del cambio en la variable de impacto bajo los distintos posibles efectos de $Z$ sobre $D$. Así, el segundo término de la suma se anula porque su multiplicador es cero: corresponde a aquellos para quienes la asignación no modificó el cumplimiento. Noten que esto **no** dice que el efecto de tratamiento de esos individuos sea nulo, sino que la asignación no los mueve y por eso no contribuyen al efecto de $Z$ sobre $Y$. Usando el supuesto 5 de monotonicidad, sabemos que $D_i(1)-D_i(0)$ es igual a uno o cero, es decir, $P(D_i(1)-D_i(0)=-1)=0$, con lo que el tercer término también desaparece.
