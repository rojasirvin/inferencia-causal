...
$$\text{Efecto de la adopción en }Y=\frac{\text{Efecto de la asignación en }Y}{\text{Efecto de la asignación en la adopción}}$$
...
- La asignación de $Z_i$ es aleatoria, es decir, $P(Z=C)=P(Z=C')$ para todo par de vectores de asignación $C$ y $C'$ **con el mismo número de tratados**
...
- Es decir, permite escribir $Y$ en función solo de $D$
...
$$
\begin{aligned}
E(Y_i(1,D_i(1))-Y_i(0,D_i(0)))&=E\left[(Y_i(1)-Y_i(0))(D_i(1)-D_i(0))\right]\\
&= (1)\cdot E(Y_i(1)-Y_i(0)|D_i(1)-D_i(0)=1)P(D_i(1)-D_i(0)=1)\\
&+(0)\cdot E(Y_i(1)-Y_i(0)|D_i(1)-D_i(0)=0)P(D_i(1)-D_i(0)=0)\\
&+(-1)\cdot E(Y_i(1)-Y_i(0)|D_i(1)-D_i(0)=-1)P(D_i(1)-D_i(0)=-1)\\
\end{aligned}
$$

- Los factores $(1)$, $(0)$ y $(-1)$ son los tres valores que puede tomar $D_i(1)-D_i(0)$. Al condicionar en cada uno de ellos, ese segundo factor del producto se vuelve una constante y sale de la esperanza

- Cada valor corresponde a un tipo de individuo: $(1)$ a los **cumplidores**, $(0)$ a quienes la asignación no mueve —**nunca cumplidores** y **siempre adoptadores**— y $(-1)$ a los **retadores**
- El segundo término de la suma es cero y corresponde a aquellos para quienes la asignación no modificó el cumplimiento
- Usando el supuesto 5 de monotonicidad, sabemos que $D_i(1)-D_i(0)$ es igual a uno o cero, es decir, 
$P(D_i(1)-D_i(0)=-1)=0$
...
$$E(Y_i(1)-Y_i(0)|D_i(1)-D_i(0)=1)=\frac{E(Y_i(1,D_i(1))-Y_i(0,D_i(0)))}{E(D_i(1)-D_i(0))}=\lambda_{LATE}$$
...
- $Z$ es independiente de $D$ para los nunca cumplidores y los siempre adoptadores y el efecto causal de $Z$ sobre $Y$ es 0 para ambos grupos
...
- Al conjunto de nunca cumplidores, siempre adoptadores y retadores se le conoce como **no cumplidores**
...
- Por el supuesto 5, sabemos que la proporción de cumplidores es igual al efecto de $Z$ sobre $D$
...
- **Primera etapa**: es el efecto causal de la asignación sobre el cumplimiento
...
::: {.fragment}
- Se muestra como el ITT puede no reflejar la efectividad de la intervención debido al efecto de *dilución* provocado, en este caso, por desviaciones con respecto a la forma de enfrentar un episodio policíaco
...
t <- asignacion %>%
  kbl(align = "lccc",
      ...
      full_width = FALSE) %>%
  kable_paper(c("hover", "condensed", "responsive")) %>%
        add_header_above(c(" ", "Tratamiento recibido:"=2, " "),
                   bold = TRUE) %>% 
...
- ¿Qué observamos sobre las desviaciones con respecto al tratamiento asignado?
...
- Por tanto, si comparamos $y$ entre aquellos individuos apercibidos y no apercibidos, no tomamos en cuenta que aquellos no apercibidos (por tanto, arrestados) pudieran ser más violentos, subestimando el efecto de la política de apercibimiento
...
- Los datos de los autores reportan que si nos fijáramos solo en las diferencias por tipo de tratamiento obtendríamos, $E(y_i|D_i=1)-E(y_i|D_i=0)=0.216-0.129=0.087$, 60% del LATE
...
- Ahora supongamos que tenemos un **instrumento** $z_i$ tal que $z_i\perp \varepsilon_i$, es decir, $E(\varepsilon_i|z_i)=0$
...
- Noten que esta derivación supuso efectos **constantes**, $y_{1i}-y_{0i}=\lambda$ para todo $i$ (un supuesto más fuerte del que necesitamos)

- Si los efectos son heterogéneos, el cociente **sigue siendo válido**, pero ya no estima el efecto promedio de toda la población sino el promedio entre los **cumplidores**, es decir, el LATE que derivamos en la sección anterior

- La aritmética de Wald no cambia, lo que cambia es **sobre quién habla** el número
...
donde $\phi_1$ es el efecto causal de la asignación sobre el cumplimiento
- Cuando $D_i$ y $z_i$ son ambas dicotómicas, $\phi_1$ da la proporción de la población de cumplidores
...
- Nota: el estimador de Wald es igual al estimador de MC2E cuando no hay $X$ y cuando $Z$ es binaria ($D$ no necesita serlo)
...
$$
\begin{aligned}
y_i&=X_i'\beta+\lambda(X_i'\phi_0+\phi_1z_i+\eta_i)+\varepsilon_i \\
&=X_i'\rho_0+\rho_1z_i+\nu_i
\end{aligned}
$$
...
- Mientras que también habíamos encontrado (3): $E(y_i|Z_i=1)-E(y_i|Z_i=0)=0.211-0.097=0.114$
...
## LATE es igual al ATET en este caso
...
- En cambio, cuando se indica **no apercibir**, es decir, arrestar, la adopción es casi perfecta (salvo un caso)

::: {.fragment}
- En otras palabras, en este caso no hay **siempre adoptadores**
...
donde $C_i=1$ para los cumplidores
